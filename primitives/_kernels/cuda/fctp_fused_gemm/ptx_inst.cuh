...
#define asm_cp_async_ca_ig(SMEM_ADDR, GMEM_ADDR, _Byte, IGNORE_SRC)                                                    \
    asm volatile("cp.async.ca.shared.global [%0], [%1], %2, %3;\n"                                                     \
                 :                                                                                                     \
                 : "r"(SMEM_ADDR), "l"(GMEM_ADDR), "n"(_Byte), "r"(IGNORE_SRC)                                         \
                 : "memory")
...
#define asm_cp_async_bulk_global_shared(GMEM_ADDR, SMEM_ADDR, _Byte)                                                   \
    asm volatile("cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;\n"                                       \
                 :                                                                                                     \
                 : "l"(GMEM_ADDR), "r"(SMEM_ADDR), "n"(_Byte)                                                          \
                 : "memory")

#define asm_cp_async_bulk_tensor_2d_global_shared(TMAP_ADDR, SMEM_ADDR, COORD_M, COORD_N)                              \
    asm volatile("cp.async.bulk.tensor.2d.global.shared::cta.tile.bulk_group"                                          \
                 " [%0, {%2, %3}], [%1];"                                                                              \
                 :                                                                                                     \
                 : "l"(TMAP_ADDR), "r"(SMEM_ADDR), "r"(COORD_N), "r"(COORD_M)                                          \
                 : "memory")

#define asm_cp_async_bulk_commit_group() asm volatile("cp.async.bulk.commit_group;\n" ::)

#define asm_cp_async_bulk_waitgroup(_N) asm volatile("cp.async.bulk.wait_group %0;\n" : : "n"(_N))

#define asm_cp_async_bulk_waitgroup_read(_N) asm volatile("cp.async.bulk.wait_group.read %0;\n" : : "n"(_N))

#define asm_fence_proxy_async_shared_cta() asm volatile("fence.proxy.async.shared::cta;\n" ::)

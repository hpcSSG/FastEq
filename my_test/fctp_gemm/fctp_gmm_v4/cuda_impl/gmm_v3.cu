...
#ifdef __FCTP_DEBUG__
    if (blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && m_count == 0)
    {
        if (threadIdx.y == 1)
            printf("[Kernel Debug]: n_count=%u, tid=%u, smem_x=%u, smem_y=%u\n", n_count, tid, smem_x, smem_y);
    }
#endif
...
__global__ void fused_gmm_kernel_v3(const __grid_constant__ CUtensorMap out_map, // 输出tensor map
...
)
{
    ...
    constexpr uint32_t stage = 1; // 流水级
...
#pragma unroll
    ...
    // 主循环 (前两次)
    uint32_t loop_k;
    for (loop_k = 0; loop_k < (V - TILE_K); loop_k += TILE_K)
    {
        // G2S
#pragma unroll
        for (uint32_t m_count = 0; m_count < W_LD16B_M_COUNTS; ++m_count)
        {
            WarpATileG2SDirect16BAsync<WARP_PER_BLOCK>(A_smem, y, t_ld_m0, t_ld_ak0, loop_k,
                                                       (W_LD16B_M_ROWS * WARP_PER_BLOCK), m_count, V, tid, wid);
        }
#pragma unroll
        for (uint32_t k_count = 0; k_count < W_LD16B_N_COUNTS; ++k_count)
        {
            WarpBTileG2SSwizzleAsync<WARP_PER_BLOCK>(B_smem, w, t_ld_n0, t_ld_bk0, loop_k,
                                                     (W_LD16B_N_ROWS * WARP_PER_BLOCK), k_count, b_path_id, U, V, W,
                                                     tid, wid);
        }
...
#pragma unroll
        for (uint32_t _n = 0; _n < (W_TILE_N / MMA_N); ++_n)
        {
            WarpBSubtileN8K4S2RSwizzleSync<W_TILE_N / 8>(B_reg[_n], B_smem, _n, tid, w_nid);
        }
        ...
        for (uint32_t _m = 0; _m < (W_TILE_M / MMA_M); ++_m)
        {
            ...
            double A_reg[2];
            WarpASubtileM8K4S2RDirectSync<W_TILE_M / 8>(A_reg[0], A_smem, (2 * _m), tid, w_mid);
            WarpASubtileM8K4S2RDirectSync<W_TILE_M / 8>(A_reg[1], A_smem, (2 * _m + 1), tid, w_mid);
...
#pragma unroll
        for (uint32_t m_count = 0; m_count < W_LD16B_M_COUNTS; ++m_count)
        {
            ...
            WarpATileG2SReverse8BAsync<WARP_PER_BLOCK>(A_smem, y, t_ld_m1, t_ld_ak1, loop_k,
                                                       (W_LD16B_M_ROWS * WARP_PER_BLOCK), m_count, V, tid, wid);
        }
...
#pragma unroll
        ...
        for (uint32_t _m = 0; _m < (W_TILE_M / MMA_M); ++_m)
        {
            ...
            {
                WarpASubtileM8K4S2RReverseSync<W_TILE_M / 8>(A_reg[0], A_smem, (2 * _m), tid, w_mid);
                WarpASubtileM8K4S2RReverseSync<W_TILE_M / 8>(A_reg[1], A_smem, (2 * _m + 1), tid, w_mid);
            }
            // MMA
#pragma unroll
            for (uint32_t _n = 0; _n < (W_TILE_N / MMA_N); ++_n)
...
)
{
    ...
    constexpr uint32_t SMEM_SIZE = ((TILE_M * TILE_K + TILE_N * TILE_K) * 1 // 1 stage load buffer
                                    + (TILE_M * TILE_N))                    // store buffer
                                   * sizeof(double);

    auto cuda_func = fused_gmm_kernel_v3<M_WARPS, N_WARPS, W_TILE_M, W_TILE_N, TILE_K>;

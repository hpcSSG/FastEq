...
__global__ void fused_gmm_kernel_v2(const double *__restrict__ x, // [B, total_i, U]
                                    ...
                                    int32_t* i_dims,                // [num_paths]
                                    int32_t* prefex_i_sum,          // [num_paths]
...
#pragma unroll
    ...
    {
        b_path_id = (b_i_id >= prefex_i_sum[_p] && b_i_id < prefex_i_sum[_p + 1]) ? _p : b_path_id;
    }
    uint32_t b_path_size = i_dims[b_path_id];
    uint32_t b_prefix_i_sum = prefex_i_sum[b_path_id];
...
#pragma unroll
    ...
    std::vector<int32_t> i_dims(i_dims_vec.size(), 0);
    std::vector<int32_t> prefix_i_sum(i_dims_vec.size(), 0);
    for (int idx=0; idx<i_dims_vec.size(); idx++) {
        i_dims[idx] = i_dims_vec[idx];
        if (idx>0)
            prefix_i_sum[idx] = prefix_i_sum[idx - 1] + i_dims_vec[idx - 1];
    }

    torch::Tensor i_dims_gpu = torch::tensor(i_dims_vec, x.options().dtype(torch::kInt32)).contiguous();
    torch::Tensor prefix_gpu = torch::tensor(prefix_i_sum, x.options().dtype(torch::kInt32)).contiguous();
    ...
    cuda_kernel<<<grid, block, SMEM_SIZE, cur_stream>>>(x.data_ptr<double>(), w.data_ptr<double>(),
                                                        out.data_ptr<double>(), i_dims_gpu.data_ptr<int32_t>(), prefix_gpu.data_ptr<int32_t>(), num_paths,
                                                        total_i, B, U, V, val);

    return out;
}

TORCH_LIBRARY(fused_gmm, m)
{

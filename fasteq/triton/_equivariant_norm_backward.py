...
def _parameter_partial(X, DY, MU, RSTD, PW, PB, GROUP,
                       ...
                       BIAS_GRAD: tl.constexpr, BM: tl.constexpr, BC: tl.constexpr):
...
def _fused_backward(X, DY, MU, RSTD, DX, PW, PB, COEFFICIENT, DEGREE, W0, WH,
                    ...
                    GROUP_BOUNDS: tl.constexpr, BK: tl.constexpr, BC: tl.constexpr):
    n = tl.program_id(0)
    ...
    if WEIGHT_GRAD:
        product = dy * zhat
        # Gamma is shared by m within each degree, independently of stats groups.
...
def backward(op, x, mean, rstd, weight0, weight_high, dy, needs):
    ...
    pw = empty((n, s.lmax + 1, c)) if nw else None
            ...
            if op.version == 'v0':
                if nx:
                    ...
                    _group_dot[(n, s.num_stats)](x, dy, ptr(mean), rstd, q, op.bounds,
                        ...
                        ORDER=op.reduction_order, BK=op.block_k, **launch)
                ...
                if nw or nb:
                    _parameter_partial[(n, s.lmax + 1)](x, dy, ptr(mean), rstd, ptr(pw),
                        ...
                        BM=triton.next_power_of_2(2 * s.lmax + 1), **launch)
            elif nx or nw or nb:

...
__global__ void uniform1d_combine_u224_p215_bwd(
...
template <typename scalar_t>
void launch_uniform1d_combine_u224_p215_bwd(
    const scalar_t* w,
    ...
    cudaStream_t stream)
{
    dim3 block(32);
    dim3 grid(B, 1);
    uniform1d_combine_u224_p215_bwd<scalar_t><<<grid, block, 0, stream>>>(
...
std::vector<torch::Tensor> launcher_uniform1d_combine_u224_p215_bwd(
    ...
    int64_t V64)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(w.scalar_type(), "uniform1d_combine_u224_p215_bwd", [&] {

        launch_uniform1d_combine_u224_p215_bwd<scalar_t>(
...
TORCH_LIBRARY(uniform1d_combine_u224_p215_bwd_codegen, m) {
    m.def("run", &launcher_uniform1d_combine_u224_p215_bwd);
}

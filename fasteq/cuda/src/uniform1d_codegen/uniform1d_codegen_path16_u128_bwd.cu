...
__global__ void uniform1d_codegen_blocku_vgroup_path16_u128_bwd(
...
template <typename scalar_t>
void launch_uniform1d_codegen_blocku_vgroup_path16_u128_bwd(
    const scalar_t* w,
    ...
    cudaStream_t stream)
{
    ...
    dim3 block(U);
    dim3 grid(B);
    uniform1d_codegen_blocku_vgroup_path16_u128_bwd<scalar_t><<<grid, block, smem_bytes, stream>>>(

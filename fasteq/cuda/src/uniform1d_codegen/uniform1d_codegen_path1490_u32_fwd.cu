...
__global__ void uniform1d_codegen_path1490_u32_fwd(
...
// launcher helper
template <typename scalar_t>
void launch_uniform1d_codegen_path1490_u32_fwd(
    const scalar_t* w,
    ...
    cudaStream_t stream)
{
    ...
    dim3 grid(B, (U + 31) / 32);
    uniform1d_codegen_path1490_u32_fwd<scalar_t><<<grid, block, 0, stream>>>(
        w, x_all, y, out, src_idx, dst_idx, b_list,
        B, Iw, Ix, Ky, V, U);
}


torch::Tensor launcher_uniform1d_codegen_path1490_u32_fwd(
    ...
    int64_t V64)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(w.scalar_type(), "uniform1d_codegen_path1490_u32_fwd", [&] {

        launch_uniform1d_codegen_path1490_u32_fwd<scalar_t>(
...
TORCH_LIBRARY(uniform1d_codegen_path1490_u32_fwd_codegen, m) {
    m.def("run", &launcher_uniform1d_codegen_path1490_u32_fwd);
}

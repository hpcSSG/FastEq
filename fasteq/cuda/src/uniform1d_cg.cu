...
__global__ void stp_codegen_two_warp_vgroup(
...
torch::Tensor u1d_codegen_fwd_launch(
    ...
    int64_t V64)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(w.scalar_type(), "u1d_fused_fwd_atomic", [&]{
        ...
        dim3 grid(B);
        stp_codegen_two_warp_vgroup<scalar_t><<<grid, block, 0, stream>>>(
...
TORCH_LIBRARY(u1d_cg, m)
{
    m.def("forward", &u1d_codegen_fwd_launch);
}
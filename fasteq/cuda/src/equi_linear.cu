                                       ...
                                       double val)
{
    ...
    {
        mutipath_equi_linear_f32_impl(num_paths, static_cast<float *>(out_ptr), static_cast<float *>(x_ptr),
                                      static_cast<float *>(w_ptr), B, total_i, i_dims_vec, U, V, val, cur_stream);
    }
    else
    {
        mutipath_equi_linear_f64_impl(num_paths, static_cast<double *>(out_ptr), static_cast<double *>(x_ptr),
...
torch::Tensor mutipath_equi_linear_bwd(const torch::Tensor &x,                 // [B, total_i, U]
                                       const torch::Tensor &w,                 // [num_paths, U, V]
                                       const std::vector<int64_t> &i_dims_vec, // i dims
                                       double val)
{

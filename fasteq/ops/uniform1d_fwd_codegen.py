...
def emit_two_warp_vgroup_forward_kernel(
    ...
    ap("    int64_t y_base = (int64_t)b   * (int64_t)Ky;")
    ap("")
...
def generate_code_uniform1d_fwd(
    ...
    kernel_name = f"uniform1d_combine_u{u_dim}_path{P}_fwd"
    ...
    if u_dim == 32:
        code = emit_two_warp_vgroup_forward_kernel_u32(groups, kernel_name=kernel_name, scalar_t=scalar_t)
    else:

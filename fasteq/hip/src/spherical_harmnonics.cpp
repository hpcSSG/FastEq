  ...
  for (int j = warpID; j < blockDim.x; j += 8) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      for (int offset = 8; offset > 0; offset /= 2) {
        prod += __shfl_down(FULL_MASK, prod, offset);
      }
...
std::vector<torch::Tensor> spherical_harmonics_forward(torch::Tensor xyz) {
    ...
    AT_DISPATCH_FLOATING_TYPES(
        xyz.scalar_type(), "spherical_harmonics_kernel_ptr", ([&] {
        bool _norm = true;
        bool _grad = false;
        ...
        hipError_t err = hipFuncSetAttribute((const void *)spherical_harmonics_kernel_ptr<scalar_t>, hipFuncAttributeMaxDynamicSharedMemorySize, (int)space);
        TORCH_CHECK(err == hipSuccess, "hipFuncSetAttribute failed: ", hipGetErrorString(err));

        hipStream_t stream = at::hip::getCurrentHIPStream();

        spherical_harmonics_kernel_ptr<scalar_t><<<gdim, bdim, space, stream>>>(
            xyz.data_ptr<scalar_t>(),
            sph_harmonics.data_ptr<scalar_t>(),
            sph_harmonics_deriv.data_ptr<scalar_t>(),
            _nsamples,
            _norm,
            _grad);
...
torch::Tensor spherical_harmonics_backward(torch::Tensor sph_deriv,
  ...
  AT_DISPATCH_FLOATING_TYPES(
      sph_deriv.scalar_type(), "spherical_harmonics_backward_kernel_ptr", ([&] {
        ...
        //hipError_t err = hipFuncSetAttribute( spherical_harmonics_backward_kernel_ptr<scalar_t>, hipFuncAttributeMaxDynamicSharedMemorySize, (int)space);
        //TORCH_CHECK(err == hipSuccess, "hipFuncSetAttribute failed: ", hipGetErrorString(err));

        hipStream_t stream = at::hip::getCurrentHIPStream();
        spherical_harmonics_backward_kernel_ptr<scalar_t><<<gdim, bdim, space, stream>>>(
            sph_deriv.data_ptr<scalar_t> (),
...
TORCH_LIBRARY(fast_spherical_harmonics, m)
{
    m.def("forward", &spherical_harmonics_forward);
    m.def("backward", &spherical_harmonics_backward);
}
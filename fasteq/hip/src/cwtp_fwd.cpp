...
#define HIP_CHECK(expr)                                                   \
    do {                                                                   \
        ...
        TORCH_CHECK(_err == hipSuccess,                                   \
                    "HIP error: ", hipGetErrorString(_err),              \
...
std::tuple<at::Tensor, at::Tensor> cwtp_mace_small_forward(
    ...
    TORCH_CHECK(x.is_hip() && y.is_hip() && w.is_hip(), "HIP tensors required");
    ...
    fwd_mace_small_kernel<<<grid, block, 0, at::hip::getCurrentHIPStream()>>>(
        x.data_ptr<double>(),
        y.data_ptr<double>(),
        w.data_ptr<double>(),
        out.data_ptr<double>(),

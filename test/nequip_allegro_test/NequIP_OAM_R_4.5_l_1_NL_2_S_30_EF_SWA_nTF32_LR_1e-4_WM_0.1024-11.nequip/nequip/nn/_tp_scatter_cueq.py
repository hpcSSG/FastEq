...
class CuEquivarianceTensorProductScatter(TensorProductScatter):
    def __init__(
        ...
        tp_desc = nequip_tp_desc(
                cue.Irreps(O3_e3nn, feature_irreps_in),
                cue.Irreps(O3_e3nn, irreps_edge_attr),
                cue.Irreps(O3_e3nn, irreps_mid),
            )
        tp_desc_flatten_squeeze = tp_desc.flatten_coefficient_modes().squeeze_modes()

        u1d_compatible = (
            tp_desc_flatten_squeeze.all_same_segment_shape()
            and len(tp_desc_flatten_squeeze.polynomial.operations[0][1].subscripts.modes()) == 1
        )

        print(f"feature_irreps_in:{feature_irreps_in}") 
        print(f"irreps_edge_attr:{irreps_edge_attr}") 
        print(f"irreps_mid:{irreps_mid}") 
        print(f"tp_desc: {tp_desc}")
        print(f"tp_desc flatten squeeze: {tp_desc_flatten_squeeze}")

        if not u1d_compatible:
            self.fasteq_tp_conv = cuet.FastEqSegmentedPolynomial(
                nequip_tp_desc(
                    cue.Irreps(O3_e3nn, feature_irreps_in),
                    cue.Irreps(O3_e3nn, irreps_edge_attr),
                    cue.Irreps(O3_e3nn, irreps_mid),
